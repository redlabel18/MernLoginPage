    ...
    <nav className="w-full flex justify-between items-center p-4 sm:p-6 sm:px-24 absolute top-0">
      ...
      {userData ? (
        <div className=" flex justify-center items-center w-8 h-8 rounded-full bg-black text-white relative group">
          ...
          <div className="absolute hidden group-hover:block top-0 right-0 z-10 text-black rounded-full pt-10">
            <ul className="list-none m-0 p-2 bg-gray-100 text-sm">
              {!userData.isAccountVerified && <li onClick={sendVerificationOtp} className="py-1 px-2 cursor-pointer hover:bg-gray-200">Verify Email</li>}
              
              <li onClick={logout} className="py-1 px-2 cursor-pointer hover:bg-gray-200 pr-10">Logout</li>
            </ul>
          </div>
        </div>
      ) : (
        <button
          ...
          className="flex items-center gap-2 border border-gray-500 text-gray-800 rounded-full px-6 py-2 hover:bg-gray-100 cursor-pointer transition-all"
        >
